...
function love.load()
	...
	pack = player:addSlot(newSlot("Back",true,{"large","backpack","satchel","jetpack","canister","tool"}))
	
	head = accessory:addSlot(newSlot("Head",false,{"helmet","hat","anysize"}))
	eyes = accessory:addSlot(newSlot("Eyes",false,{"glasses","visor","anysize"}))
	...
	hands = simple:addSlot(newSlot("Hands",false,{"gloves","anysize"}))
	ears = simple:addSlot(newSlot("Ears",false,{"earpiece","anysize"}))
	
	body = complex:addSlot(newSlot("Body",false,{"clothing","anysize"}))
	bodyexo = complex:addSlot(newSlot("Body external",false,{"armor","jacket","anysize","canister"}))
	shoes = complex:addSlot(newSlot("Feet",false,{"shoes","boots","anysize"}))
	mask = complex:addSlot(newSlot("Face",false,{"mask","anysize"}))
	...
	table.insert(ground,newItem("Screwdriver",{"tool","small"}))
	table.insert(ground,newItem("Wire Cutters",{"tool","small"}))
	table.insert(ground,newItem("Multitool",{"tool","small"}))
	...
	table.insert(ground,newItem("Crowbar",{"tool","large"}))
	table.insert(ground,newItem("Laser Rifle",{"gun","large"}))
	table.insert(ground,newItem("Revolver",{"gun","small"}))
	table.insert(ground,newItem("Pen",{"tool","small"}))
	...
	table.insert(ground,newItem("Welder",{"tool","small"}))
	table.insert(ground,newItem("Welding Gas",{"tool","small"}))
	table.insert(ground,newItem("Welding Mask",{"mask","large"}))
	table.insert(ground,newItem("Gas Mask",{"mask","small"}))
	table.insert(ground,newItem("Oxygen Tank",{"canister","large"}))
	table.insert(ground,newItem("Emergency O2 Tank",{"canister","small"}))
	table.insert(ground,newContainerItem("Rucksack",{"backpack","large"},5,{"anytype","small"}))
	table.insert(ground,newContainerItem("Toolbelt",{"belt","large"},4,{"tool","anysize"}))
...
function newSlot(name,autostow,stores)
	local o = {}
	o.type = "slot"
	o.name = name
	o.autostow = autostow
	...
	elseif type(tonumber(key)) == "number" then
		...
		if slot.visible then
			...
			elseif SHIFT then
				...
				if slot.item and not freeHand() then
...
function love.draw()
	...
	for i,v in ipairs(ground) do
		lg.print(v.name,lg.getWidth()/4*3,h)
		h = h + 2 + fontsml:getHeight()
	end
